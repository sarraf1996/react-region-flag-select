...
var MainSelect = function (_Component) {
    ...
    function MainSelect(props) {
        ...
        _this.setCountryData = function (data, flag) {
            ...
            _this.setState({ dropData: { data: data.name, flag: flagData, phoneCode: data.phoneCode }, selectedCountry: data.id, selectedState: 'State', selectedCity: 'City' });
        ...
        _this.setStateData = function (data) {
            ...
            _this.setState({ cityData: cityData, selectedCity: 'City', completeStateCode: data });
        ...
        _this.changeData = function (e) {
            ...
            _this.setState({ countries: newCountry, selectedState: 'State' });
        };
        ...
        _this.state = { completeCountryCode: null, completeStateCode: null, data: '', selectedCountry: 'IN', selectedState: 'State', selectedCity: 'City', dropData: { data: 'select a country', flag: null, phoneCode: null }, countries: customCountries.length ? customCountries : countries.countries, stateData: stateArray.length ? stateArray : [], cityData: citiesValue, countryCode: countryData };
    ...
    _createClass(MainSelect, [{
        ...
        value: function render() {
            ...
            return React.createElement(
                ...
                React.createElement(
                    'span',
                    { className: 'd-inline-block separator-span' },
                ),
                this.props.isCity && React.createElement(
                    ...
                    React.createElement(
                        'button',
                        { style: this.state.selectedState === 'State' && this.props.isState || !this.state.selectedCountry ? { padding: '6px 20px 6px 10px', opacity: '0.65' } : { padding: '6px 20px 6px 10px' } },
                        this.state.selectedCity
                    ),
                    React.createElement(
                        'label',
                        null,
                        React.createElement('input', { type: 'checkbox', style: this.state.selectedState === 'State' && this.props.isState || !this.state.selectedCountry ? { cursor: 'not-allowed' } : {} }),
                        this.state.selectedState === 'State' && this.props.isState || !this.state.selectedCountry ? '' : React.createElement(
                            'ul',
                            null,
                            React.createElement(
                                'li',
                            ...
                            React.createElement(
                                'div',
                                { style: { maxHeight: '343px', overflowY: 'auto', minHeight: '41px', marginTop: '-4px', paddingTop: '15px', paddingLeft: '10px', textAlign: '-webkit-auto' } },
                                this.state.selectedState && this.state.selectedState !== 'State' || !this.props.isState ? this.state.cityData.length ? this.state.cityData.map(function (data, idx) {
                                    return React.createElement(
                                        'li',
                                        { key: idx, onClick: function onClick() {
